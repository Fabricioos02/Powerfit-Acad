document.getElementById('cadastroForm').addEventListener('submit', async function (e) {
    ...
    try {
        ...
        const response = await fetch('https://(seu-link-do-backend)/register', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ nome, email, password, telefone, plano }),
        });
        if (!response.ok) {
        ...
        mensagemCadastroDiv.className = 'error-message'; 
        console.log(error);
